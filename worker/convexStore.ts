...
export class ConvexStore {
  ...
  async sessionExists(options: { session: string }) {
    const session = await this.client.query(api.sessions.getByOwner, {
  ...
  async extract(options: { session: string }) {
    try {
      ...
      // 1. Get file URL
      const fileUrl = await this.client.query(api.storage.getFileUrl, {
      ...
      // 3. Write zip file to the location RemoteAuth expects
      const sessionPath = (options as any).path || path.join(process.cwd(), `${options.session}.zip`);
      ...
      console.log(`✅ Cloud session downloaded to ${sessionPath}`);
    } catch (error: any) {
      console.warn(
        `⚠️  Failed to download session from cloud: ${error.message}`,
      );
    }
  }
